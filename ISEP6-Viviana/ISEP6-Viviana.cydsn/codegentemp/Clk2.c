...
void Clk2_Start(void) 
{
    ...
    Clk2_CLKEN |= Clk2_CLKEN_MASK;
	Clk2_CLKSTBY |= Clk2_CLKSTBY_MASK;
}
...
void Clk2_Stop(void) 
{
    ...
    Clk2_CLKEN &= (uint8)(~Clk2_CLKEN_MASK);
	Clk2_CLKSTBY &= (uint8)(~Clk2_CLKSTBY_MASK);
}
...
{
    if ((Clk2_CLKEN & Clk2_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Clk2_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clk2_CLKEN_MASK;
        ...
        oldDivider = CY_GET_REG16(Clk2_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clk2_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clk2_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Clk2_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Clk2_GetSourceRegister();
    uint16 oldDivider = Clk2_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clk2_CLKEN & Clk2_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clk2_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
{
    return CY_GET_REG16(Clk2_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clk2_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Clk2_MOD_SRC |= modeBitMask & (uint8)Clk2_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clk2_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Clk2_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clk2_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clk2_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Clk2_MOD_SRC & (uint8)(Clk2_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clk2_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Clk2_GetDividerRegister();
    uint8 oldSrc = Clk2_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clk2_MOD_SRC |= CYCLK_SSS;
        Clk2_MOD_SRC =
            (Clk2_MOD_SRC & (uint8)(~Clk2_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clk2_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Clk2_MOD_SRC & Clk2_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clk2_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Clk2_PHASE = clkPhase & Clk2_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clk2_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Clk2_PHASE & Clk2_PHASE_MASK;
}
