...
void Clk_1MHz_Start(void) 
{
    ...
    Clk_1MHz_CLKEN |= Clk_1MHz_CLKEN_MASK;
	Clk_1MHz_CLKSTBY |= Clk_1MHz_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Clk_1MHz_Stop
********************************************************************************
*
* Summary:
...
void Clk_1MHz_Stop(void) 
{
    ...
    Clk_1MHz_CLKEN &= (uint8)(~Clk_1MHz_CLKEN_MASK);
	Clk_1MHz_CLKSTBY &= (uint8)(~Clk_1MHz_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clk_1MHz_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Clk_1MHz_CLKEN & Clk_1MHz_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Clk_1MHz_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clk_1MHz_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clk_1MHz__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clk_1MHz_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clk_1MHz_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clk_1MHz_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Clk_1MHz_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Clk_1MHz_GetSourceRegister();
    uint16 oldDivider = Clk_1MHz_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clk_1MHz_CLKEN & Clk_1MHz_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clk_1MHz_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Clk_1MHz_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Clk_1MHz_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clk_1MHz_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Clk_1MHz_MOD_SRC |= modeBitMask & (uint8)Clk_1MHz_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clk_1MHz_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Clk_1MHz_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clk_1MHz_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clk_1MHz_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Clk_1MHz_MOD_SRC & (uint8)(Clk_1MHz_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clk_1MHz_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Clk_1MHz_GetDividerRegister();
    uint8 oldSrc = Clk_1MHz_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clk_1MHz_MOD_SRC |= CYCLK_SSS;
        Clk_1MHz_MOD_SRC =
            (Clk_1MHz_MOD_SRC & (uint8)(~Clk_1MHz_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clk_1MHz_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Clk_1MHz_MOD_SRC & Clk_1MHz_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clk_1MHz_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Clk_1MHz_PHASE = clkPhase & Clk_1MHz_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clk_1MHz_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Clk_1MHz_PHASE & Clk_1MHz_PHASE_MASK;
}
