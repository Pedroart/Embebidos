...
void Clk_100Hz_Start(void) 
{
    ...
    Clk_100Hz_CLKEN |= Clk_100Hz_CLKEN_MASK;
	Clk_100Hz_CLKSTBY |= Clk_100Hz_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Clk_100Hz_Stop
********************************************************************************
*
* Summary:
...
void Clk_100Hz_Stop(void) 
{
    ...
    Clk_100Hz_CLKEN &= (uint8)(~Clk_100Hz_CLKEN_MASK);
	Clk_100Hz_CLKSTBY &= (uint8)(~Clk_100Hz_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clk_100Hz_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Clk_100Hz_CLKEN & Clk_100Hz_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Clk_100Hz_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clk_100Hz_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clk_100Hz__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clk_100Hz_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clk_100Hz_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clk_100Hz_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Clk_100Hz_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Clk_100Hz_GetSourceRegister();
    uint16 oldDivider = Clk_100Hz_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clk_100Hz_CLKEN & Clk_100Hz_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clk_100Hz_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Clk_100Hz_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Clk_100Hz_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clk_100Hz_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Clk_100Hz_MOD_SRC |= modeBitMask & (uint8)Clk_100Hz_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clk_100Hz_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Clk_100Hz_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clk_100Hz_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clk_100Hz_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Clk_100Hz_MOD_SRC & (uint8)(Clk_100Hz_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clk_100Hz_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Clk_100Hz_GetDividerRegister();
    uint8 oldSrc = Clk_100Hz_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clk_100Hz_MOD_SRC |= CYCLK_SSS;
        Clk_100Hz_MOD_SRC =
            (Clk_100Hz_MOD_SRC & (uint8)(~Clk_100Hz_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clk_100Hz_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Clk_100Hz_MOD_SRC & Clk_100Hz_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clk_100Hz_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Clk_100Hz_PHASE = clkPhase & Clk_100Hz_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clk_100Hz_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Clk_100Hz_PHASE & Clk_100Hz_PHASE_MASK;
}
